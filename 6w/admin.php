...
$user = 'u17634';
$pass = '4532343';
$db = new PDO('mysql:host=localhost;dbname=u17634', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['delete'])) {
        ...
        setcookie('owner_id', $member_id, time() + 12 * 30 * 24 * 60 * 60);
        ...
        try {
            ...
            foreach ($select as $value) {
                $stmt = $db->prepare("SELECT id from teampowers WHERE name = ?");
...
if (!empty($_SERVER['PHP_AUTH_USER']) && !empty($_SERVER['PHP_AUTH_PW'])) {
    ...
    $stmt = $db->prepare("SELECT pow.name AS name, count(*) AS amount FROM powersowners JOIN teampowers pow ON power_id = pow.id GROUP BY power_id");
    $stmt->execute();
