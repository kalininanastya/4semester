...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
    ...
    if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
        try {
            ...
            $stmt->execute(array($member));
            $owner_id = $stmt->fetch(PDO::FETCH_ASSOC);

            $superpowers = $db->prepare("DELETE FROM powersowners WHERE owner_id = ?");
            $superpowers->execute(array($owner_id['id']));
            ...
            foreach ($powers as $value) {
                ...
                $superpowers->execute(array($power_id['id'], $owner_id['id']));
            }
        } catch (PDOException $e) {
            print('Error : ' . $e->getMessage());
            exit();
        ...
        try {
            ...
            foreach ($powers as $value) {
                ...
                $superpowers = $db->prepare("INSERT INTO powersowners SET power_id = ?, owner_id = ? ");
                $superpowers->execute(array($power_id['id'], $owner_id));
            }
        } catch (PDOException $e) {
            print('Error : ' . $e->getMessage());
            exit();
